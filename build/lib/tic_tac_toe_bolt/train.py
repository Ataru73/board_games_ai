...
class TrainPipeline:
    ...
    def policy_update(self):
        ...
        # But wait, get_equi_data returns just the board (3,3).
        # We need to reconstruct the 3-channel input.
        # But we don't have 'current_player' info in get_equi_data easily unless we store it.
        # Actually, in self-play we stored (board, current_player, probs).
        # But in get_equi_data we just took state.
        # And state was just board.
        # We need to know whose turn it was to construct the input correctly?
        # Actually, we can just assume the board is always from the perspective of the player who is about to move?
        # In AlphaZero, the board is usually canonicalized.
        # Let's canonicalize the board in start_self_play before storing.
        # i.e. multiply board by current_player so that 1 is always "me" and -1 is "opponent".
        
        # Let's fix start_self_play to store canonical board.
        
        # ... (Need to fix start_self_play)
        
        # Assuming we fixed it:
        state_batch_tensor = torch.zeros(self.batch_size, 3, 3, 3)
        for i, board in enumerate(state_batch):
            state_batch_tensor[i, 0, :, :] = (board == 1)
            state_batch_tensor[i, 1, :, :] = (board == -1)
